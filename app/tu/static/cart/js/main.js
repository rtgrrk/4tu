	...
	if (cart.length > 0) {
		...
		function addProduct(target) {
			...
			<li class="cd-cart__product">
				<div class="cd-cart__image"><a href="#${name}"><img src="${image}" alt="placeholder"></a></div>
				<div class="cd-cart__details">
					<h3 class="truncate"><a href="#${name}">${title}</a></h3>
					<span class="cd-cart__price">&euro;${price}</span>
					<div class="cd-cart__actions">
						<a href="#${name}" class="cd-cart__delete-item">Delete</a>
					</div>
				</div>
				</li>
			`;
		...
		function removeProduct(product) {
			...
			var topPosition = product.offsetTop,
				productTotPrice = Number((product.getElementsByClassName('cd-cart__price')[0].innerText).replace('&euro;', ''));
		...
		function quickUpdateCart() {
			...
			for (var i = 0; i < cartListItems.length; i++) {
				if (!Util.hasClass(cartListItems[i], 'cd-cart__product--deleted')) {
					...
					price = price + singleQuantity * Number((cartListItems[i].getElementsByClassName('cd-cart__price')[0].innerText).replace('&euro;', ''));
				}
			}
